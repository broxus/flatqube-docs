...
* [_Express (Node.js)_ ](https://expressjs.com/en/4x/api.html)__
* __[_Axios_](https://axios-http.com/docs/intro)__
* __[_body-parser_](https://www.npmjs.com/package/body-parser)__
...
This function helps you to get farming pools information:\
\
...
`pairLink`: URL to the certain farming pool where you can see all the details about that pool and start farming in it\
`logo`: \
...
It can be used for representing data specific for each currency in detail.

Retrieves following data about the certain currency:\
...
Get currencies prices in USD by token root address/addresses. \
...
****Retrieves all desired currencies based on their addresses and other request body parameters. \
It can be used to show all currencies and their data in the list format.

Data retrieved: \
`count`: number of currencies per page \
`currencies`: list of all retrieved currencies with following data: \
...
`offset`: offset \
`ordering`: weather the currencies should be ordered by TVL ascending or descending `whiteListUri`: path to the white list
...
It can be used for graphic representation of prices change over a certain period of time.
...
Body required **** de_scribed in the "data" field._
...
Get currency volume data info. It can be used ie. for graphic representation of trading volume (in USD) change for the required period of time. \
Retrieves following information: \
`data`: trading volume for the given moment inside of the given timespan, based on the timeframe (hourly, daily etc.) \
...
Body required d_escribed in the "data" field._
...
__`timeframe`: desired timeframe to retrieve trading volume data, could be set for hours, days, etc. (“H1”, “D1”...) \
...
Retrieves following information: \
`data`: TVL for the given moment inside of the given timespan, based on the timeframe (hourly, daily etc.) \
`timestamp`: date time of the retrieved trading volume data
...
Retrieves list of all the main trading volume data for a given timespan (from, to) in a certain timeframe (each hour, each day…). \
It can be used as well for a graphical representation of trading volumes over some period of time or any kind of monitoring the trading volume.
...
&#x20;Retrieves list of all the main TVL (total value locked) data for a given timespan (from, to) in a certain timeframe (each hour, each day…). It can be used as well for a graphical representation of trading volumes over some period of time or any kind of monitoring the TVL.&#x20;
...
Get all Pairs data. \
It can be used for ie. table view of all the pairs and desired info about them. \
Retrieves following:\
...
****It can be used for presenting one pair with all the details about them in the desired pool. \
...
__It can be used anywhere where certain details about one currency and how it affects the other one in the pair and vice versa is needed. \
...
Data used for Postman tests: \
`fromCurrencyAddress`: currency address for which to retrieve cross pairs \
`toCurrencyAddresses`: currency address/addresses we are checking weather it is a pair with currency in fromCurrencyAddress parameter
...
Retrieves an object with following data:\
`meta`: object containing following data: \
\- `base`: left token name (ie. WEVER) \
\- `baseAddress`: root address of the left token \
\- `counter`: right token name (ie. BRIDGE) \
...
`tvlChange`: TVL change (percentage) in the pool for the last 24h \
`leftLocked`: tvl of the left pair (token) in USD \
`leftPrice`: price (in USD) of the left pair `rightLocked`: tvl of the right pair (token) in USD \
`rightPrice`: price (in USD) of the left pair \
`volume24h`: trading volume of the pair in the last 24h (in USD) \
`volumeChange24h`: trading volume change (percentage) in the last 24h \
`volume7d`: trading volume of the pair in the last 7 days (in USD) \
...
**POST request**\
****l_eft parameter and right parameter required - represents address of a specific currency._ \
...
__It can be used ie. for graphic representation of price change of the right pair compared to the value of the left pair over a certain period of time (ie. 1 WEVER = 0.02245627 BRIDGE).\
Retrieves a list of prices data each for a given timeframe (H1, D1…) in a desired period of time (from, to) for a right pair:\
`close`: value of the currency (in USD) in the last moment of given timeframe \
`closeTimestamp`: date time for the “close” value in UNIX format \
`high`: the highest currency price in the given timeframe \
...
Data used for Postman tests:\
__`from`: date time in UNIX format for the start of timeframe (ie. 1646741858511 or March 8, 2022 12:17:38.511 PM GMT time) \
`timeframe`: desired timeframe to retrieve prices data, could be set for hours, days, etc. (“H1”, “D1”...) \
`to`: date time in UNIX format for the end of timeframe (ie. 1647346658513 or March 15, 2022 12:17:38.513 PM)
...
It can be used ie. for graphic representation of trading volume changes in the pair’s pool over a certain period of time.\
...
Get Transactions data of a certain user filtered by required parameters. \
...
`createdAtLe`: date time in UNIX format to filter all transaction that were created before or during that date time \
